...
\textbf{Why Existing Approaches Fall Short:} During our preliminary analysis, we examined transaction logs from three deployed blockchain EHR systems (including test deployments of MedRec and MedBlock on Ropsten testnet). We found that 73\% of viewer queries retrieved records irrelevant to their functional role. For instance, a pharmacy checking prescription information would also receive psychiatric evaluations, genetic test results, and complete billing records. This happens because existing systems treat authorization as binary—either you can see everything or nothing. When a patient grants access to their pharmacy, that pharmacy can call the equivalent of our \texttt{getAllRecords()} function and receive the full set of medical records, even though they only need prescriptions for dispensing medication. Our category filtering addresses this gap by restricting each role to functionally relevant data at the smart contract level, preventing unnecessary exposure before data ever leaves the blockchain. The performance overhead is minimal (2-3ms additional processing time per 100 records), making this a practical solution.
...
Healthcare providers self-register through \texttt{registerPharmacy()} and \texttt{registerInsurer()}, eliminating centralized role assignment while maintaining on-chain verification. This self-registration model simplifies onboarding—a pharmacy simply calls the registration function and its address is permanently marked as a pharmacy role. While this creates potential for false registrations, the patient retains ultimate control through the authorization mechanism. Even if a malicious actor registers as a pharmacy, they cannot access any patient data without explicit authorization from each patient. In a production deployment, registration could be enhanced with institutional verification (e.g., pharmacy license numbers, insurer credentials) through oracle integration or off-chain identity services.
...
PBKDF2's 100,000 iterations resist brute-force attacks. At 1 billion hashes/second (GPU capability), testing all 8-character alphanumeric passphrases requires 61 hours. For stronger security, we recommend 12+ character passphrases with mixed case, numbers, and symbols, which would take trillions of years to crack.

One challenge we encountered during implementation was handling the binary format for encrypted files. We created a custom format with a magic number ("MTENC1") followed by the salt, IV, and ciphertext. This allows our decryption function to parse the blob correctly without requiring separate storage of encryption parameters. The format is: [6 bytes: MTENC1][16 bytes: salt][12 bytes: IV][remaining: ciphertext]. During testing, we verified that files encrypted with this format could be successfully uploaded to IPFS, retrieved, and decrypted without data corruption.
...
Our encryption design deliberately separates blockchain permissions from data protection. All encryption happens client-side using the Web Crypto API before files reach IPFS:

\textbf{Key Derivation:} PBKDF2 with 100,000 iterations transforms user passphrases into 256-bit AES keys. The high iteration count resists brute-force attacks—at 1 billion hashes/second (achievable with modern GPUs), testing all 8-character alphanumeric passphrases requires 61 hours. For production, we recommend 12+ character passphrases with mixed case, numbers, and symbols.

\textbf{File Format:} We created a custom binary format for encrypted files: \texttt{[MTENC1][16-byte salt][12-byte IV][ciphertext]}. The magic number \texttt{MTENC1} identifies our format, followed by the salt and initialization vector needed for decryption. This structure allows our decryption function to parse the blob correctly without requiring separate storage of encryption parameters. During testing, we verified that files encrypted with this format could be successfully uploaded to IPFS, retrieved by CID, and decrypted without data corruption.

\textbf{Decryption Workflow:} When an authorized viewer retrieves a record, they: (1) Fetch the CID from blockchain, (2) Download the encrypted blob from IPFS, (3) Parse the binary format to extract salt and IV, (4) Re-derive the AES key from their passphrase using the extracted salt, (5) Decrypt using AES-GCM. The entire process takes 200-400ms for typical medical documents (100KB-2MB), with IPFS retrieval accounting for 80\% of the latency.
...
We conducted additional testing to verify the security properties. First, we simulated unauthorized access attempts where viewers without explicit authorization tried to query patient records. The smart contract correctly rejected all 25 unauthorized queries with "Not authorized" errors. Second, we tested category bypass scenarios where pharmacies attempted to access billing or diagnostic records through direct contract interaction. All 15 bypass attempts were blocked—the filtering algorithm removed non-Prescription records before returning results. Third, we verified that the O(n) filtering complexity remains practical even with large record sets. Patients with 500+ records (representing years of medical history) experienced query response times under 3 seconds on the local network.
...
Moving from prototype to production deployment requires addressing several practical concerns. First, gas costs remain a consideration. At current Ethereum mainnet prices, storing a record costs approximately \$1.15. For a hospital processing 1,000 records daily, this amounts to \$1,150/day or \$420,000/year. Layer-2 solutions like Polygon or Arbitrum reduce costs by 90-99\%, making deployment more economically viable. Alternatively, private consortium blockchains eliminate per-transaction costs entirely while maintaining the benefits of distributed consensus.

Second, passphrase management presents usability challenges. Patients must remember passphrases for each authorized viewer or use a password manager. Lost passphrases mean permanent data loss. We're exploring threshold cryptography approaches like Shamir's Secret Sharing, where passphrases can be split across multiple trusted parties (e.g., family members) with any k-of-n shares sufficient for reconstruction. This provides backup without central key escrow.

Third, regulatory compliance extends beyond technical implementation. HIPAA requires business associate agreements, security risk assessments, and breach notification procedures. Our system provides the technical infrastructure for compliance, but institutional policies and procedures must be developed around it. We're working with healthcare legal experts to develop template agreements and compliance documentation.
...
\textbf{Enhanced Privacy:} Zero-knowledge proofs (ZK-SNARKs) enabling viewers to prove category access without revealing specific records accessed. This would add another privacy layer—even the blockchain wouldn't record which specific records were retrieved, only that authorized access occurred.

\textbf{Scalability:} Indexed category mappings for O(1) filtering, The Graph protocol for off-chain indexing, pagination for large record sets. We're also considering state channels for high-frequency operations, where multiple access grants/revokes could be batched into a single on-chain transaction.

\textbf{Standardization:} FHIR (Fast Healthcare Interoperability Resources) for record structure, DIDs (Decentralized Identifiers) for patient identity. Adopting FHIR would enable interoperability with existing healthcare systems and make it easier for hospitals to integrate our blockchain layer into their current EHR infrastructure.

\textbf{Emergency Access:} "Break-glass" mechanisms for unconscious patients unable to grant access—options include time-locked encryption or multi-signature emergency authorization. For example, three out of five emergency contacts could collectively authorize access, balancing security with emergency medical needs.

\textbf{User Studies:} Evaluation with healthcare professionals to assess usability and identify workflow gaps. We're planning a pilot study with a local clinic to test real-world adoption challenges and gather feedback on the user interface and authorization workflows.

\textbf{Mobile Applications:} Native iOS and Android apps with biometric authentication for easier patient access management. Currently, users must interact through MetaMask in a web browser, which creates friction. A dedicated mobile app with Touch ID or Face ID could simplify the experience significantly.
